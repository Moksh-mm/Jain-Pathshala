...
// ============================================
// Helper Functions (Needed locally)
// ============================================
...
router.get('/report', async (req, res) => {
  // Access dependencies passed via req.app.locals
  const { getCollection, ALL_STUDENTS } = req.app.locals;
  ...
  try {
    ...
    // Fetch dynamic users to include in export
    const usersCollection = await getCollection('users');
    let allStudentsMap = { ...ALL_STUDENTS };
    
    if (usersCollection) {
      const dbUsers = await usersCollection.find({ role: 'student' }).toArray();
      dbUsers.forEach(u => {
        const key = u.username.toLowerCase();
        if (!allStudentsMap[key]) {
          allStudentsMap[key] = { username: u.username, name: u.name };
        }
      });
    }

    const studentsData = await Promise.all(
      Object.values(allStudentsMap).map(async (student) => {
